Feature: A12_EstSignedShareRangeValidation

@Sanity @Regression
Scenario: Estimated Signed Share has to be between +100 or -100
...
# Validation : A12_EstSignedShareRangeValidation
	Then User capture the value of 'Starr EPI/GWP'
	Then User Clicked on "Edit" button
	Then User Updated 'Policy Premium Estimated Signing %' field
	Then User clicked on Save button during Edit Mode in Stage Progression for backtracking
	Then Verify the Error messages for Backtracking as "Estimated Signing is required and must be between +/- 100% ( Premium and Limits Tab )"
