Feature: A92_Lock_Child_Subm_Fields

@Sanity @Regression
Scenario: Issuing Company on the Renewal Submission should match the Issuing Company on the Related Submission. If Section or Terrorism or Endorsement,should match the Parent Submission.Applicable March 17 Onwards.
...
# Validaiton : A92_Lock_Child_Subm_Fields
	Then User Clicked on "New Endorsements" button
	And User Fill the values in Endorsement creation page for "Endorsement"
	Then User Updated 'Issuing Company,Carrier Branch' field with values 'Starr Indemnity & Liability Company,Japan'
	Then User Updated 'FAC RI PRM In Child Records' field
	Then User clicked on save button for validation
	Then Verify the Error messages for Backtracking as "Issuing Company / Carrier Branch on the Child Submission should match the value on the Parent Submission. Contact Administrator otherwise"
...
# Create Section 
	...
	Then User Updated 'FAC RI PRM In Child Records' field
	Then User Updated 'Issuing Company,Carrier Branch' field with values 'Starr Indemnity & Liability Company,Japan'
	#Then User Updated 'Production Office' field
	Then User clicked on save button for validation
	Then Verify the Error messages for Backtracking as "Issuing Company / Carrier Branch on the Child Submission should match the value on the Parent Submission. Contact Administrator otherwise"
