Feature: A108_Bound_Date_Required


@Sanity @Regression
Scenario: Bound Date field under Submission Detail section should be required if issuing office = Bratislava, Rotterdam, Budapest, or Zurich.1.0
...
#  Validation : A108_Bound_Date_Required
	Then User capture the value of 'Starr EPI/GWP'
	Then User Clicked on "Edit" button
	When User performed Stage progression from "Quoted" to "Bound"
	Then User Updated 'Issuing Office' field with values 'Prague'
	Then User clicked on Save button during Edit Mode in Stage Progression for backtracking
	Then Verify the Error messages for Backtracking as "Bound Date is required when Issuing office is Bratislava, Budapest, Madrid, Malta, Munich, Prague, Rotterdam, or Zurich. ( Dates/Additional Info )"
