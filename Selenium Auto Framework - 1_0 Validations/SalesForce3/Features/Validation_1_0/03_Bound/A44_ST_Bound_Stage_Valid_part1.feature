Feature: A44_ST_Bound_Stage_Valid_part1
...
#  Validation : A44_ST_Bound_Stage_Valid_part1
	Then User capture the value of 'Starr EPI/GWP'
	Then User Clicked on "Edit" button
	When User performed Stage progression from "Quoted" to "Bound"
	Then User Updated 'Slip Position' field with values '--None--'
	Then User clicked on Save button during Edit Mode in Stage Progression for backtracking
	Then Verify the Error messages for Backtracking as "Required for Bound stage. - Field ( Policy Information Tab - Slip Position )"
