Feature: A142_Starr_TRIA_Non_Cert_Req


@Sanity @Regression
Scenario: Either Starr TRIA or Non Cert Premium field must be populated if TRIA/Non Cert Included = Yes OR TRIA fields cannot be updated once the submission has been fed to LYNX ( Starr TRIA, Non Cert Premium : Policy Info )
...
#  Validation : A142_Starr_TRIA_Non_Cert_Req
	Then User capture the value of 'Starr EPI/GWP'
	Then User Clicked on "Edit" button
	When User performed Stage progression from "Quoted" to "Bound"
	Then User Updated 'TRIA/Non Cert Included,Starr TRIA Premium,Starr Non Cert Premium' field with values 'Yes,"",""'
	Then User clicked on Save button during Edit Mode in Stage Progression for backtracking
	Then Verify the Error messages for Backtracking as "Either Starr TRIA or Non Cert Premium field must be populated if TRIA/Non Cert Included = Yes OR TRIA fields cannot be updated once the submission has been fed to LYNX ( Starr TRIA, Non Cert Premium : Policy Info )"
